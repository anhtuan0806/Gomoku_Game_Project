...
/** @file BotAI.cpp
 *  @brief Triển khai AI Bot: Zobrist hashing, Transposition Table và Alpha-Beta
 * search.
 */
 ...
 *
 * @note Chỉ ghi đè khi bucket rỗng hoặc depth mới >= depth cũ để giữ giá trị
 * sâu hơn.
 */
static inline void storeTranspositionEntry(std::uint64_t hash, int score,
...
/**
 * @brief Phân tích một hướng (dirRow,dirCol) bắt đầu từ ô (row,col) cho
 * `player`.
 * @return LineInfo chứa số quân liên tiếp và số đầu hở.
 * @note `openEnds` chỉ tính khi ô kế tiếp nằm trong bàn VÀ trống.
 */
static LineInfo
analyzeDirection(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE], int size,
                 int row, int col, int dirRow, int dirCol, int player) {
  LineInfo res = {1, 0};

  // Hướng tiến
  int forwardRow = row + dirRow, forwardCol = col + dirCol;
  while (isInBounds(forwardRow, forwardCol, size) &&
         board[forwardRow][forwardCol] == player) {
    res.count++;
    forwardRow += dirRow;
    forwardCol += dirCol;
  }
  // Đầu hở: phải trong bàn VÀ ô đó trống (không phải quân địch, không phải
  // biên)
  if (isInBounds(forwardRow, forwardCol, size) &&
      board[forwardRow][forwardCol] == CELL_EMPTY)
    res.openEnds++;

  // Hướng lùi
  int backwardRow = row - dirRow;
  int backwardCol = col - dirCol;
  while (isInBounds(backwardRow, backwardCol, size) &&
         board[backwardRow][backwardCol] == player) {
    res.count++;
    backwardRow -= dirRow;
    backwardCol -= dirCol;
  }
  if (isInBounds(backwardRow, backwardCol, size) &&
      board[backwardRow][backwardCol] == CELL_EMPTY)
    res.openEnds++;
 ...
 */
static int scoreMove(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE], int size,
                     int winLen, int row, int col, int player) {
  static const int dirs[4][2] = {{0, 1}, {1, 0}, {1, 1}, {1, -1}};
  int total = 0;
  for (int dirIdx = 0; dirIdx < 4; dirIdx++) {
    LineInfo line = analyzeDirection(board, size, row, col, dirs[dirIdx][0],
                                     dirs[dirIdx][1], player);
...
/**
 * @brief Kiểm tra xem ô (row,col) có phải là candidate (gần một quân nào đó
 * trong radius 2).
 */
static bool isCandidate(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE],
  ...
  for (int deltaRow = -2; deltaRow <= 2; deltaRow++)
    for (int deltaCol = -2; deltaCol <= 2; deltaCol++) {
      ...
      if (isInBounds(neighborRow, neighborCol, size) &&
          board[neighborRow][neighborCol] != CELL_EMPTY)
        return true;
    }
  return false;
}
 ...
 */
static int
computeIncrementalScore(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE],
                        int size, int winLen) {
  int score = 0;
 ...
 */
static bool hasImmediateWin(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE],
    ...
    for (int col = 0; col < size; col++) {
      ...
      for (int dirIdx = 0; dirIdx < 4; dirIdx++) {
        ...
        for (int step = 1; step < winLen; step++) {
          int nextRow = row + dirs[dirIdx][0] * step,
              nextCol = col + dirs[dirIdx][1] * step;
          if (isInBounds(nextRow, nextCol, size) &&
 ...
 */
static bool
hasImmediateWinAroundMove(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE],
                          int size, int winLen, int lastRow, int lastCol,
                          int player) {
  static const int dirs[4][2] = {{0, 1}, {1, 0}, {1, 1}, {1, -1}};
  for (int dirIdx = 0; dirIdx < 4; dirIdx++) {
    int consecutiveCount = 1;
 ...
 */
static int alphaBetaSearch(int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE], int size,
                           int winLen, int depth, int alpha, int beta,
                           bool isMaximizing, std::uint64_t hash, int lastRow,
                           int lastCol) {
  ...
  if (lastRow >= 0 && lastCol >= 0) {
    ...
    if (lastPlayer != CELL_EMPTY &&
        hasImmediateWinAroundMove(board, size, winLen, lastRow, lastCol,
                                  lastPlayer)) {
      return (lastPlayer == CELL_PLAYER2) ? (900000 + depth)
                                          : (-900000 - depth);
    }
  }
  ...
  for (int row = 0; row < size; row++) {
    for (int col = 0; col < size; col++) {
      ...
      int defenseScore =
          scoreMove(board, size, winLen, row, col, opponentPlayer);
  ...
  for (const auto &m : moves) {
    std::uint64_t newHash =
        hash ^ zobristHashForCell(m.row, m.col,
                                  isMaximizing ? CELL_PLAYER2 : CELL_PLAYER1);
    board[m.row][m.col] = isMaximizing ? CELL_PLAYER2 : CELL_PLAYER1;

    int value = alphaBetaSearch(board, size, winLen, depth - 1, alpha, beta,
                                !isMaximizing, newHash, m.row, m.col);
 ...
 */
static std::uint64_t buildHash(const int board[MAX_BOARD_SIZE][MAX_BOARD_SIZE],
                               int size) {
  std::uint64_t h = 0;
  for (int row = 0; row < size; row++)
    for (int col = 0; col < size; col++)
...
/**
 * @brief Lấy danh sách các nước đi tiềm năng và sắp xếp theo điểm heuristic.
 */
static std::vector<MoveCandidate> getSortedCandidates(const PlayState *state,
                                                      int winLen) {
  ...
  for (int row = 0; row < size; row++) {
    for (int col = 0; col < size; col++) {
      if (state->board[row][col] != CELL_EMPTY ||
          !isCandidate(state->board, size, row, col))
        continue;

      int attackScore =
          scoreMove(state->board, size, winLen, row, col, CELL_PLAYER2);
      int defenseScore =
          scoreMove(state->board, size, winLen, row, col, CELL_PLAYER1);

      int totalScore = 0;
      // Ưu tiên 1: Thắng ngay lập tức
      if (attackScore >= 1000000) {
        totalScore = 2000000;
      }
      // Ưu tiên 2: Chặn đối thủ thắng ngay lập tức
      else if (defenseScore >= 1000000) {
        totalScore = 1500000;
      }
      // Bình thường: Tổng điểm tấn công và phòng thủ (phòng thủ quan trọng hơn
      // một chút)
      else {
        totalScore = attackScore + defenseScore * 2;
      }

      moves.push_back({row, col, totalScore});
    }
  }

  // Nếu bàn cờ trống hoàn toàn (nước đi đầu tiên), chọn giữa bàn
  if (moves.empty()) {
...
/**
 * @brief Chọn nước đi cho bot Dễ (Easy): Chặn/thắng nếu cần, nếu không thì chọn
 * ngẫu nhiên trong top 5.
 */
static void selectEasyMove(const PlayState *state, int &outRow, int &outCol) {
  int winLen = (state->gameMode == MODE_CARO) ? 5 : 3;
  std::vector<MoveCandidate> moves = getSortedCandidates(state, winLen);

  if (moves.empty())
    return; // Không thể xảy ra do getSortedCandidates luôn trả về ít nhất 1
            // nước

  // Nếu có nước đi bắt buộc (thắng hoặc chặn thua), ưu tiên chọn ngay
  if (moves[0].score >= 1500000) {
    outRow = moves[0].row;
    outCol = moves[0].col;
    return;
  }

  // Nếu không, chọn ngẫu nhiên trong top 5 nước đi tốt nhất
  int topK = std::min((int)moves.size(), 5);
  int randomIndex = std::rand() % topK;
  outRow = moves[randomIndex].row;
  outCol = moves[randomIndex].col;
}

/**
 * @brief Chọn nước đi cho bot Trung Bình (Medium): Luôn chọn nước đi có
 * heuristic cao nhất.
 */
static void selectMediumMove(const PlayState *state, int &outRow, int &outCol) {
  int winLen = (state->gameMode == MODE_CARO) ? 5 : 3;
  std::vector<MoveCandidate> moves = getSortedCandidates(state, winLen);
...
/**
 * @brief Tính toán nước đi cho máy dựa trên `difficulty` và trạng thái hiện
 * tại.
 *
 * @param state Trạng thái ván chơi hiện thời.
 * @param difficulty 1..3 (1 Easy, 2 Medium, 3 Hard).
 * @param outRow Tham chiếu trả về hàng nước đi.
 * @param outCol Tham chiếu trả về cột nước đi.
 */
void calculateComputerMove(const PlayState *state, int difficulty, int &outRow,
                           int &outCol) {
  initializeZobrist();

  if (difficulty == 1) {
    selectEasyMove(state, outRow, outCol);
    return;
  }
  if (difficulty == 2) {
    selectMediumMove(state, outRow, outCol);
    return;
  }
  ...
  for (int row = 0; row < size; row++) {
    for (int col = 0; col < size; col++) {
      if (virtualBoard[row][col] != CELL_EMPTY ||
          !isCandidate(virtualBoard, size, row, col))
        continue;
      int attackScore =
          scoreMove(virtualBoard, size, winLen, row, col, CELL_PLAYER2);
      int defenseScore =
          scoreMove(virtualBoard, size, winLen, row, col, CELL_PLAYER1);
  ...
  for (const auto &m : moves) {
    std::uint64_t newHash =
        rootHash ^ zobristHashForCell(m.row, m.col, CELL_PLAYER2);
    virtualBoard[m.row][m.col] = CELL_PLAYER2;

    int score = alphaBetaSearch(virtualBoard, size, winLen, depth - 1, INT_MIN,
                                INT_MAX, false, newHash, m.row, m.col);
