...
static int evaluateLine(const LineInfo& line, int winLen)
{
    ...
    {
        return 1000000; // Thắng tuyệt đối
    }
    if (line.openEnds == 0)
    {
        return 0;         // Bị chặn 2 đầu thì vô dụng
    }
...
static int evaluateBoard(PlayState* state)
{
    ...
    for (int r = 0; r < state->boardSize; r++)
    {
        ...
        {
            if (state->board[r][c] == CELL_PLAYER2)
            {
                score += scoreMove(state, r, c, CELL_PLAYER2);
            }
            else if (state->board[r][c] == CELL_PLAYER1)
            {
                score -= scoreMove(state, r, c, CELL_PLAYER1);
            }
        }
    }
    return score;
