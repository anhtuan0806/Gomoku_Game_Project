...
\setlength{\parindent}{1.5em} Ở mức trung bình và tại nút gốc của mức khó, điểm phòng thủ được nhân hệ số lớn hơn (\texttt{def * 2}) để ưu tiên chặn thua sớm; trong các tầng alpha-beta sâu hơn, trọng số công/thủ cân bằng hơn (\texttt{atk + def}).
...
\setlength{\parindent}{1.5em} Để giảm không gian tìm kiếm, AI chỉ xét các ô trống nằm trong bán kính 2 ô quanh những quân đã có. Trong cây alpha-beta, số lượng nước đi mỗi tầng bị giới hạn ở \texttt{MAX\_BRANCHES = 12}; riêng nút gốc vẫn duyệt đầy đủ danh sách ứng cử viên đã sắp xếp. Nhờ đó, độ sâu 5 vẫn chạy được trong thời gian thực.
...
\begin{itemize}
	...
	\item Bước 3: tại các tầng alpha-beta, giữ lại tối đa \texttt{MAX\_BRANCHES} nước đi tốt nhất.
\end{itemize}
