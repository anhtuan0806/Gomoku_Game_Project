...
\setlength{\parindent}{1.5em} Trường hợp hòa được xử lý ngay trong \texttt{processMove}: nếu \texttt{checkWinCondition} trả về \texttt{CELL\_EMPTY}, ván được kết thúc với \texttt{winner = CELL\_EMPTY} để UI hiển thị kết quả hòa.
...
\setlength{\parindent}{1.5em} Hệ thống thời gian quản lý hai trục độc lập: đếm ngược cho từng lượt (\texttt{timeRemaining}) và tổng thời gian giữ bóng (\texttt{totalTimePossessed}). Tại vòng lặp PlayScreen, mỗi khung hình cộng dồn thời gian cho người đang tới lượt; phần đếm ngược và xử lý timeout chỉ áp dụng cho PvP (\texttt{MATCH\_PVE} bỏ qua \texttt{UpdateCountdown}). Nhờ vậy, luật chơi vẫn chuẩn xác ngay cả khi tốc độ khung hình dao động.
...
\setlength{\parindent}{1.5em} Redo thực hiện ngược với Undo: lấy nước đi từ \texttt{redoStack}, đặt lại lên bàn, cập nhật lịch sử, rồi kiểm tra thắng. Nếu phát hiện kết thúc ván, hệ thống cập nhật trạng thái và người thắng, đồng thời tăng \texttt{totalWins} tương ứng; nếu chưa kết thúc thì đổi lượt bình thường. Ở chế độ PvE, redo cũng tiến 2 nước để trả lượt về người chơi.
