...
% \usepackage{mathpazo} % Bỏ để tránh chồng phông với fontspec
...
\usepackage{tabularx}
\usepackage{caption}
\setmainfont[Scale=1.00]{Times New Roman} % hoặc font có hỗ trợ tiếng Việt
...
\newcommand{\cppkw}[1]{\textcolor{blue}{#1}}
\newcommand{\cpptype}[1]{\textcolor{blue}{#1}}
\lstset{ 
...
% Dùng \captionsetup{type=lstlisting} trong table để đưa vào Danh sách mã nguồn
...
% Tùy chỉnh màu sắc cho link (figure/table/listing refs, TOC, ...)
\hypersetup{colorlinks=true, linkcolor=black, citecolor=blue, urlcolor=fitblue}